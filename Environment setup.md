...
yстановка [Homebrew](https://it-here.ru/instruktsii/kak-ustanovit-homebrew-na-mac/)
[статья](https://lifehacker.ru/homebrew/)
...
(дает Chocolatey необходимые разрешения, чтобы он не запрашивал их каждый раз при установке программ)
...
Также, команда `code` теперь должна быть известна системе (быть в PATH) и быть доступной для использования в командной строке. Т.е. мы можем открыть какой-либо файл из консоли, написав `code <путь к файлу>`. Проверка доступности команды: `code --version`, `code --help`.
Если не работает, попаробуйте перезагрузить терминал.
...
`Shell Command: Install 'code' command in PATH command`. и выполните ее (т.е.нажмите). Перезапустите терминал.
...
При использовании VS Code (как и большинства других IDE) важно открывать не отдельные файлы, а именно целые папки с вашим проектами.
В Windows в контекстном меню папки появляется команда Open in VS Code.
Чтобы добавить аналогичную функцию в VS Code:
- 1й метод: разархивируйте папку Open.in.VS.Code.workflow.zip (содержит папку Open in VS Code.workflow) в папку: "~/Library/Services". См. [здесь](https://github.com/microsoft/vscode/issues/1223)
...
> настройки расширений можно увидеть, если на вкладке extensions в сайдбаре нажать на шестеренку у соответствующего расширения. внизу будет пункт extensions settings (если настройки для данного расширения вообще предусмотрены).
...
VScode скачает и установит набор расширений, после чего предложит перезагрузить окно, чтобы применить изменения (соглашайтесь).
...
При необходимости (если в командной строке появится запрос на подтверждение) нажмите A, чтобы подтвердить. Такое может быть, если вы забыли выполнить [этот](#Предоставление-разрешений-chocolatey) пункт
...
2. появится сообщение VERBOSE: About to download package from 'https://github.com/PowerShell/PowerShell/releases/download/v7.0.0/PowerShell-7.0.0-win-x64.msi' и индикатор занрузки, ждем.

...
Но, в связи с тем, что загрузка настроек _перезаписывает_ локальный файл настроек, лучше всего это делать только сразу после первой установки VS Code для задания начальных настроек. В дальнейшем ваши настройки будут меняться по вашему вкусу, поэтому начальные настройки содержат id другого Gist, который содержит только список расширений. Поэтому задействуем двухэтапную схему: вы подключаетесь к первому Gist - загружаете настройки - и тем самымт подключаетесь вместо первого ко второму Gist, с которым и работаете дальше.

...
projects
	danIT
		...
		fem7
```
Избегайте русских символов и пробелов в именах папок и файлов, используйте - или  _ вместо пробел
...
Второе предупреждение может касаться отсутствия SSH-ключей, об этом можете не беспокоиться, SSH использовать не обязательно. просто закройте его. Если есть желание, можете настроить, как описано [здесь](#настройка-SSH-на-GitLab)
...
3. Теперь приватный ключ нужно добавить в ssh-agent, сервис, который должен быть запущен на ввашем компьютере и будет обеспечивать авторизацию с помощью этого ключа.

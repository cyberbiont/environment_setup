...
Иначе, переходим на вкладку _Extensions_ сайдбара, в поиске вводим _Live Share Extension Pack_ и устанавливаем его. Перезагружаем VS Code (Сtrl+R).
...
### Возможные проблемы
Иногда возникают проблемы с авторизацией через браузер. Если вы пытаетесь авторизироваться через GitHub и на длительное время зависли на странице "You are being Authorizing...", обратить внимание на надпись внизу "Having trouble? If you browser doesn't redirect you back, click here". Нажмите на ссылку и следуйте инструкциям (нужно будет скопировть код и вставить его в VS Code).
...
### Расшаривание сервера (localhost)
...
Live-reload сервер для проекта можно запустить, например, с помощью расширения LiveServer,  после чего "расшарить" URL, на котором он будет работать, с помощью команды Share Server. Все участники сессии получат доступ к этому серверу по URL со своих компьютеров.

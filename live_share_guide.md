...
> Для расширенного функционала, который включает в себя текстовый чат, аудиочат и визуальные подсказки, можно установить пакет расширений Live Share Extension Pack, который включает в себя сам Live Share, Live Share Chat, Live Share Audio и Peacock (расширение, позволяющее управлять цветом "рамки" в VS Code). Но т.к. Live Share Audio вызывает проблемы на некоторых системных конфигурациях (например, [эту ошибку](#Ошибка-SlimCore-not-found-/-Electron-not-found)), а для чатов на занятиях используется специальный софт, который богаче по функционалу, вероятно лучше обойтись без Live Share Chat и Live Share Audio. Эти расширения также не отображают имена студетов, присоединившихся как readonly, что усложняет их использование.
-
## Установка
...
> Если Вы хотите начать _read-only_ сессию, это можно сделать с помощью специальной команды `Start Read-only collaboration session` в Session Details на вкладке Live Share в сайдбаре.
...
1. Пройти по инвайт-линку.
На открывшейся странице браузера во всплывающем окне будет предложение открыть ссылку с помощью VS Code, его нужно принять. это запускает у него на компьютере __новый экземпляр VS Code__ с сессией. Т.к. предполагается, что у студента в текущем экземпляре VSCode открыт собственный проект, над которым он будет работать, иногда переключаясь на общую сессию, этот способ подходит лучше, чем второй.
2. Можно использовать команду `Join collaboration session` (доступна в session details на вкладке live share в сайдбаре, или среди опций при клике по имени аккаунта в статус-баре, или через Command palette). В этом случае сессия откроется __в текущем окне__ VS Code, т.е. открытый до этого проект будет закрыт и заменен сессией Live Share.

Если студент еще не авторизирован, во всплывающем окне ему будет предложено авторизироваться (Sign In), или же продолжить как гость в режиме Read-only.
...
После авторизации студент сможет редактировать код и вносить изменения в проект.
...
Live-reload сервер для проекта можно запустить, например, с помощью расширения Preview On Web Server (команда Launch on Browser, доступна в контекстном меню при правом клике по html-файлу) или расширения LiveServer.

После этого надо "расшарить" URL, на котором он будет работать, с помощью команды Share Server (есть в "быстром" меню, открывающемся при клике на ваш логин в статус-баре, а также на вкладке Live Share в сайдбаре в Session Details - Shared Servers - Share Server)
Введите в появившемся окне номер порта, на которым работает запущенный Вами сервер (например, 3000 или 8080)
Все участники сессии получат доступ к этому серверу по URL со своих компьютеров.

Преподавателю есть смысл задать кастомный порт для сервера, на котором будет "крутиться" общая сессия, во избежание конфликта с портом, на котором может работать локальный live-reload сервер у студента.

### Live Share для студентов

#### Участие в сессии
Если преподаватель просит всех присоединиться к сессии как read-only, сначала убедитесь, что Вы не залогинены.
Если Вы залогинены. Внизу в статус-баре рядом к кнопкой LiveShare будет отображаться кнопка с вашим логином. Разлогиньтесь, нажав на нее и выбрав в появившемся меню `Sign Out`.
После этого перейдите по инвайт-ссылке и во всплывающем окошке справа выберите кнопку `Read-only`.
Обязательно введите свое имя и фамилию в появившейся вверху форме и нажмите Enter. См. [присоединение к сессии](#Присоединение-к-сессии)

Если преподаватель просит вас участовать в редактировании проекта, вам нужно будет залогиниться и переподключиться к сессии, выбрав вариант `Sign In`

#### Консультация с преподавателем
Если вы хотите, чтобы преподаватель проконсультировал Вас по вашему коду, вы должны будете создать свою сессию (см. [Запуск сессии](#Запуск-сессии)).
Вам нужно будет также запустить и расшарить локальный сервер. Как это сделать, см. [здесь](#Расшаривание-сервера)
Преподавателю также может понадобиться доступ к вашему терминалу. См. [Расшаривание терминала](#Расшаривание-терминала)
...
Но, желательно также одновременно потребовать внимания через аудиочат, потому что студенты работающие в этот момент параллельно в своем проекте, могут не заметить, что происходит в общем.
...
Студентам можно дать доступ к localhost с помощью Live Share, но если нужно что-то показать, например, в DevTools, придется воспользоваться обычной демонстрацией экрана с помощью софта для видеоконференций (в данный момент используем GoToWebinar).

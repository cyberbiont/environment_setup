...
- MacOS: появляется предупреждение "... running outside application directory".
  [Решение](https://github.com/MicrosoftDocs/live-share/issues/2408#issuecomment-505640943). Проще говоря, переместить Visual Studio Code.app сначала на рабочий стол, а уже оттуда - в директорию ~/Applications.
...
- MacOS: не устанавливается Live Share.
  ...
  Проблема проявляется на MacOS и описана [здесь](https://github.com/MicrosoftDocs/live-share/issues/3124) и [здесь](https://github.com/MicrosoftDocs/live-share/issues/180)

  Что можно попробовать:

  - Переустановить Live Share или полностью весь Live Share Extension Pack (если установлен).
  ...
  - Удалить данные расширений. Зайдите в директорию ~/Library/Application Support/Code/user/globalStorage (чтобы попасть туда из папки пользователя, нужно включить отображение папки Library в настройках вида папки) и удалите папки ms-vsliveshare.vsliveshare и ms-vsliveshare.vsliveshare-audio. после этого еще раз переустановите Live Share. - Установить более раннюю версию Live Share. Нажмите на шестеренку в правом нижнем углу плашки расширения в списке расширений и выберите Install Another Version. Появится список доступных версий, выберите версию 1.0.1456 и перезагрузите VSCode.
  - Почистить кэш. В той же директории Code очистить содержимое папок, имя которых содержит в себе "cache" или "cached" перезагрузить VSCode и попробовать переустановить расширение.

- Ошибка SlimCore not found / Electron not found
  Результат бага в Live Share Audio, по поводу которого открыто масса issue на Github.
  Решение: Удалите Live Share Audio.
...
1. Вы должны будете создать свою сессию.
...
2. Вам нужно будет также запустить и расшарить локальный сервер.

   2.1. Для этого сначала запустите его (зависит от того, какое расширение вы используйте, если не знаете, как это сделать, уточните у преподавателя).

   У вас автоматически откроется вкладка в браузере с превью вашего проекта. Посмотрите в адресной строке порт, на котором работает сервер (цифра после двоеточия).

   2.2. Затем используйте команду ShareServer
